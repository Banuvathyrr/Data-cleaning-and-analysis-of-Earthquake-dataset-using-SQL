...
-- 1)  What is the distribution of earthquakes over different years and months?
SELECT 
...
SELECT 
	STATUS, 
    Count_of_status,
	(Count_of_status/ (SELECT SUM(Count_of_status) FROM CTE))*100 AS pct
FROM 
    CTE
ORDER BY pct desc;
...
SELECT 
	YEAR, COUNT(*) as Number_of_EQS, ROUND(AVG(Magnitude),2) as Avg_mag
FROM
	eqdatabase
GROUP BY 
	Year
...
FROM 
	eqdatabase
WHERE 
	Magnitude=(SELECT MAX(Magnitude) FROM eqdatabase);
